...
def synthetic_data(df):
    ...
    # Convert to PyTorch tensor and add an extra dimension to match the input_dim of the model
    ...
    model.eval()
    with torch.no_grad():
        z = StudentT(df=model.v.item()).rsample((10000, latent_dim))  # Sample from the Student's t-distribution
        generated_data = model.decode(z).numpy()

    # Post-process the generated data (denormalize back to original range if necessary)
